...
namespace UrbanApi.Controllers
{
    ...
    public class PartnerController : ControllerBase
    {
        ...
        public PartnerController(AppDbContext db, IConfiguration config)
        {
            _db = db;
            _config = config;
        }
        ...
        private async Task<(User user, Professional professional)?> GetCurrentProfessionalAsync(CancellationToken ct)
        {
        ...
        private static PartnerProfileDto MapProfile(User user, Professional professional)
        {
        ...
        [AllowAnonymous]
        [HttpPost("signup")]
        public async Task<IActionResult> Signup([FromBody] PartnerSignupRequest input, CancellationToken ct)
        {
        ...
        public async Task<IActionResult> GetProfile(CancellationToken ct)
        {
            ...
            return Ok(MapProfile(professional.Value.user, professional.Value.professional));
        }
        ...
        public async Task<IActionResult> UpdateStatus([FromBody] ProfessionalOnlineStatusDto input, CancellationToken ct)
        {
            ...
            var professional = await _db.Professionals
                ...
                .Include(p => p.City)
                .FirstOrDefaultAsync(p => p.UserId == userId && !p.IsDeleted, ct);
            if (professional?.User == null) return NotFound();

            professional.IsOnline = input.IsOnline;
            professional.UpdatedAt = DateTime.UtcNow;
            await _db.SaveChangesAsync(ct);

            return Ok(MapProfile(professional.User, professional));
        }
        ...
        public async Task<IActionResult> GetBookings([FromQuery] string? status, CancellationToken ct)
        {
            ...
            var professionalId = await _db.Professionals
                .Where(p => p.UserId == userId && !p.IsDeleted)
                .Select(p => (Guid?)p.Id)
                .FirstOrDefaultAsync(ct);
            if (professionalId == null) return NotFound();
            ...
            var query = _db.Bookings
                ...
                .Where(b => b.ProfessionalId == professionalId && !b.IsDeleted);
        ...
        public async Task<IActionResult> GetDashboard(CancellationToken ct)
        {
            ...
            var professionalId = professional.Value.professional.Id;
            ...
            var dto = new PartnerDashboardDto
            {
